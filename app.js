...
var firebaseConfig = {
    apiKey: "AIzaSyAoq0NY_3BOK704oJ0nCnrR0i5kF1P1KIo",
    authDomain: "seproject-e4a28.firebaseapp.com",
    projectId: "seproject-e4a28",
    storageBucket: "seproject-e4a28.appspot.com",
    messagingSenderId: "698804696032",
    appId: "1:698804696032:web:b50cc66964ab22abe1b185",
    measurementId: "G-K8740GXP8K"
  };
// Initialize Firebase
...
app.get('/:course_id/file/upload',(req,res)=>{
    File.find({courseID:req.params.course_id}).populate('UploadedBy').exec((err,files)=>{
        Course.findById(req.params.course_id,(err1,course)=>{
            res.render('filePage.ejs',{course_id:req.params.course_id,course:course,files:files,currentUser:req.session.currentUser});
        })   
    });
});
...
app.get('/:course_id/markfile/upload',(req,res)=>{
    if(req.session.currentUser.userType=='Teacher'){
        Course.findById(req.params.course_id,(err1,course)=>{
            Markfile.find({courseCode:course.courseCode}).exec((err,files)=>{
                res.render('markfilePage.ejs',{course_id:req.params.course_id,course:course,files:files,currentUser:req.session.currentUser});
            })
        });
    }else{
        Course.findById(req.params.course_id,(err,foundcourse)=>{
            Markfile.findOne({courseCode:foundcourse.courseCode},(err,markfile)=>{
                if(markfile){
                    ...
                    console.log(markobj);
                    res.render('studentMarkpage.ejs',{markobj:markobj,course_id:req.params.course_id,foundcourse:foundcourse});
                }
                else{
                    res.render('studentMarkpage.ejs',{markobj:markfile,course_id:req.params.course_id,foundcourse:foundcourse});
                }
            })
        })
    }
    ...
    if(req.session.currentUser.userType=='Teacher'){
        Course.findById(req.params.course_id,(err,foundcourse)=>{
            TeacherAssignment.find({courseCode:foundcourse.courseCode}).exec((err,assignments)=>{
                res.render('TeacherAssignment.ejs',{course_id:req.params.course_id,foundcourse:foundcourse,assignments:assignments,currentUser:req.session.currentUser});
            });
        })
    }else{
        Course.findById(req.params.course_id,(err,foundcourse)=>{
            TeacherAssignment.find({courseCode:foundcourse.courseCode},(err,assignments)=>{
                res.render('StudentAssignment.ejs',{course_id:req.params.course_id,foundcourse:foundcourse,assignments:assignments,currentUser:req.session.currentUser});
            })
        })
    }
})
...
app.get('/:assign_id/:course_id/assignment/submissions',(req,res)=>{
    Course.findById(req.params.course_id,(err,foundcourse)=>{
        TeacherAssignment.findById(req.params.assign_id).populate('studentList').exec((err,submissions)=>{
            if(!err){
                res.render('studentSubmissions.ejs',{submissions:submissions,foundcourse:foundcourse});
            }
        })
    })
})
